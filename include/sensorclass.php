...
class Sensor
    ...
    function getValue($conn)
		...
		$sql="SELECT round(avg(temperature),1), round(avg(humidity),1), TIMESTAMPDIFF(MINUTE,m.date_time,NOW()) as age, m.date_time, m.id FROM measure as m left join datalogger d on d.sensor=".$this->pin." and d.measureid=m.id where active = 1 group by m.id ORDER BY date_time DESC limit 1 ";
